...
fn test_config_default_values() {
    ...
    assert!(
        !config.paths.get_mount_dir().is_empty(),
        "mount_dir should have a default"
    );

    assert!(
        !config.commit.get_auto_commit_message().is_empty(),
        "auto_commit_message should have a default"
    );
    assert!(
        !config.commit.get_squash_commit_message().is_empty(),
