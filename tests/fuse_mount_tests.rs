...
fn test_fuse_real_mount_unmount() {
    let session =
        match fuse_common::FuseTestSession::with_lower_layer_setup("mount-unmount", |lower| {
...
fn test_fuse_real_mount_cleanup_on_signal() {
    let _session = match fuse_common::FuseTestSession::new("mount-cleanup") {
        Some(s) => s,
        None => return,
    };
...
fn test_fuse_real_multiple_operations() {
    let session = match fuse_common::FuseTestSession::new("multiple-ops") {
        Some(s) => s,
        None => return,
    };
